...
// constructor for software SPI - we indicate DataCommand, ChipSelect, 
Reset 
Adafruit_SSD1306::Adafruit_SSD1306(int8_t SID, int8_t SCLK, int8_t DC, 
int8_t RST, int8_t CS) : Adafruit_GFX(SSD1306_LCDWIDTH, 
SSD1306_LCDHEIGHT) {
  cs = CS;
  rst = RST;
  dc = DC;
...
// constructor for hardware SPI - we indicate DataCommand, ChipSelect, 
Reset 
...
// Hint, the display is 16 rows tall. To scroll the whole display, run:
// display.scrollright(0x00, 0x0F) 
void Adafruit_SSD1306::startscrolldiagright(uint8_t start, uint8_t 
stop){
...
void Adafruit_SSD1306::display(void) {
  ...
  ssd1306_command((SSD1306_LCDHEIGHT == 64) ? 7 : 3); // Page end 
address
  ...
  if (sid != -1)
  {
    ...
    digitalWrite(dc, HIGH);
    digitalWrite(cs, LOW);
	delayMicroseconds(1);		// May not be necessary - needs 
testing
...
inline void Adafruit_SSD1306::fastSPIwrite(uint8_t d) {
  ...
  if(hwSPI) {
    ...
    shiftOut(sid, sclk, MSBFIRST, d);		// SSD1306 specs show 
MSB out first
  }
}
...
void Adafruit_SSD1306::drawFastHLineInternal(int16_t x, int16_t y, 
...
improvement in fill* functions
    ...
    static uint8_t premask[8] = {0x00, 0x80, 0xC0, 0xE0, 0xF0, 0xF8, 
0xFC, 0xFE };
...
bits we did above
    ...
    // note - lookup table results in a nearly 10% performance 
improvement in fill* functions
    static uint8_t postmask[8] = {0x00, 0x01, 0x03, 0x07, 0x0F, 0x1F, 
0x3F, 0x7F };
